...
d={}
d["data_file1"]="/datalocal/vboxshare/THESE/CLASSIFICATION/TRAITEMENT/DATA_METEO"
d["data_file"]="/datalocal/vboxshare/THESE/CLASSIFICATION/TRAITEMENT/DATA_METEO/Temp/SAFRAN_ALL/"
d["RASTER"]= d["data_file"]+"/RASTER"
images=[]
with open(d["data_file1"]+"/list_raster_Safran.txt", 'rb+') as csvfile:
    for line in csvfile:
        print (line.rstrip())
...
for q in images:
    print (q)
    commande=commande + " %s/"%(d["RASTER"]) + str(q)[1:]
commande=commande + " -out /datalocal/vboxshare/THESE/CLASSIFICATION/TRAITEMENT/DATA_METEO/Temp/SAFRAN_ALL/RASTER_SAFRAN_2017_Concatenate.tif"
print (commande)
os.system(commande)
